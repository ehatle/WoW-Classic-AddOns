...
CTA_Util.chatPrintln = function(s)
  DEFAULT_CHAT_FRAME:AddMessage("[CTA] " ..(s or "nil"), 0.9, 0.75, 0.20);
  -- , 1, 0.75, 0.0);
end
  ...
  for i = 1, m do
    for j = 1, n do
      ...
      if ap == b then
        ...
        elseif bp == "" then
          -- Swap at first Reference letter
          d_swap =(match_score[b] or 1) + dist(nil, nil)
        end
      elseif a == bp and ap == "" then
        -- Swap at first Message letter
...
dist = function(a, b)
  if not a then
    if not b then
      -- Swap of a with b's predecessor and b with a's predecessor
      return -0.5
      -- Swap cost recommendation (swap cost < mismatch cost < 2 * gap cost)
    else
      -- deletion in A
      return -1
    end
  ...
  if a == b then
    ...
    -- Any Mismatch
    if string.find(a .. b, "[%p%s][%p%s]") then
